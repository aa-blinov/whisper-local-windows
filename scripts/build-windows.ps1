...
try {
    Write-Host '== Lazy to Text — Windows portable build =='
    ...
    if (-not $SkipSync) {
        Write-Host 'Resolving dependencies via uv sync...'
        $syncArgs = @('sync')
